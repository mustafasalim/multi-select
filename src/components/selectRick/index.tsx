    ...
    <div className="grid gap-y-6">
      <div className="relative over">
        ...
        {arrow == false ? (
          <TiArrowSortedUp
            onClick={handleArrow}
            className="absolute h-full top-0 right-0 pr-3 w-10 text-[#6e7e94] cursor-pointer"
          />
        ) : (
          <TiArrowSortedDown
            onClick={handleArrow}
      ...
      {arrow === true && (
        <div className="w-[400px] overflow-hidden rounded-s-2xl h-[500px] border-2 overflow-y-scroll transition-all custom-scrollbar border-[#6e7e94]">
          {data.length === 0 ? (
            <div>fwafw</div>
          ) : (
            <>
